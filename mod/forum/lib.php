 ...
 * than standard cron.
 *
 * NOTE: Since 3.0.1 this task is no longer one monolithic function
 */
function forum_cron() {
 ...
 * out to all subscribers.
 *
 * NOTE: Since 3.0.1 this task is no longer a part of one monolithic function
 */
function forum_mail_digests_task() {
    ...
    $digestposts = $DB->get_recordset('forum_queue');

    if ($digestposts->valid()) {
        ...
        $digestposts->close(); // Finished iteration, let's close the resultset.
        ...
        foreach ($userdiscussions as $userid => $thesediscussions) {
            ...
            $DB->delete_records_select('forum_queue', "userid = ?", array($userid));
            ...
            $headerdata->userprefs = '<a target="_blank" href="'.$headerdata->userprefs.'">'.
