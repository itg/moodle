...
// Connect to database
$conn = mysql_connect('db.midmich.edu', 'pass', '79mNgXlGLogjifu') or die("Database connection failed");
$db = mysql_select_db('moodle2_pass') or die("Database selection failed");

$sql = "SELECT COUNT(*) AS recent_users FROM mdl_user WHERE lastaccess >= " . (mktime() - (60 * 5));
...
$sql = "SELECT COUNT(*) AS recent_users FROM mdl_user WHERE lastaccess >= " . (mktime() - (60 * 60 * 3));
$rs = mysql_query($sql) or die ("Query failed: $sql");
$row = mysql_fetch_array($rs, MYSQL_ASSOC);
